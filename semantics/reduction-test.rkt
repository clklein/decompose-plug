...
(test-equal (term (inst (:in-hole (:cons (:cons :hole x) (:hide-hole (:cons y :hole)))
            ...
            (term (tuple (:cons (:cons z x) (:right y :hole))
                         :false)))
(test-equal (term (inst (:in-hole (:hide-hole :hole) x) (set)))
            (term (tuple x :false)))
(test-equal (term (inst (:in-hole (:hide-hole (:cons (:cons x :hole) y)) z) (set)))
            (term (tuple (:cons (:cons x z) y) :false)))
(test-equal (term (inst (:in-hole (:hide-hole (:cons (:cons x :hole) y)) :hole) (set)))
            (term (tuple (:left (:right x :hole) y) :true)))
(test-equal (term (inst (:in-hole (:hide-hole (:cons (:cons x :hole) y)) (:hide-hole :hole))
                        (set)))
            (term (tuple (:cons (:cons x :hole) y)
                         :false)))
