...
Ignoring embedded contexts, a term @pt[t] is simply a binary tree
where leaf nodes are atoms @pt[a] and interior nodes are constructed
with @pt[:cons]. A context @pt[C] is similarly a binary tree, but with a distinguished
path (marked with @pt[:left] and @pt[:right]) from the root of the context
to its @pt[:hole]. 

Contexts are
generated by decomposition and represent single-holed contexts. 
Although the @pt[:hole] 
can appear multiple times in a single term, such terms represent
expressions that contain multiple, independently pluggable contexts.
...
alternatives of the non-terminal @pt[n] (defined outside the pattern). 
...
The @|matches-schema/unframed| rules are organized by the structure of @pt[p].
The atom and hole rules produce an empty binding map because those pattern contain no
...
if any of the non-terminal's alternatives match. The scope of an alternative's 
pattern variables is limited to that alternative, and consequently, the 
...
the decomposition judgment form to find a decomposition and checks 
that the term in the hole matches @pt[p_2].
