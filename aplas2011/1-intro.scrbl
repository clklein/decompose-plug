...
steps may occur. This factoring enables concise specification, e.g., that
a language is call-by-value or call-by-name or call-by-need@~cite[cbn-calculus],
that @racket[if] expressions must evaluate 
the test position before the branches, and even that exceptions, continuations,
and state@~cite[felleisen-hieb] behave in the expected ways,
...
Our motivation for studying context-sensitive matching is its implementation
...
In keeping with the spirit of Redex, we augment a standard
proof-based validation of our work with testing. More concretely, in
