...
A context @pt[C] within a term carries the path to its hole in the form
...
@pt[⊔]. It combines two mappings into a single one by taking the union of their 
...
@pt[⊔] is not defined. Accordingly, rules that use @pt[⊔] apply only when
@pt[⊔] is well-defined.
