...
A term @pt[t] is, in essence, a binary tree. Leaf nodes can be either atoms (@pt[a]),
or the hole in a context. Interior nodes are constructed with one of @pt[:cons], @pt[:left],
or @pt[:right]. Matching treats all three of these constructors identically, but 
the @pt[:left] and @pt[:right] constructors indicate where the hole is and are treated
specially when plugging into a term that represents a context. 

Contexts (@pt[C]) are
generated by decomposition, and are a special case of terms.
These contexts represent single-holed contexts; although the @pt[:hole] 
